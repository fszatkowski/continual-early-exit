...
def get_loaders(
    ...
    for idx_dataset, cur_dataset in enumerate(datasets, 0):
        ...
        if max_classes_per_dataset is not None:
            if num_tasks > 1:
                raise ValueError(
                    "'max_classes_per_dataset' only works with 'num_tasks' = 1."
                )

            if cur_dataset not in ds_to_cls_order:
                ds_to_cls_order[cur_dataset] = trn_dset[0].class_indices

            trn_dset = _sample_classes(
                trn_dset,
                dataset_to_cls_idx[cur_dataset],
                dataset_to_cls_idx[cur_dataset] + max_classes_per_dataset,
            )
            val_dset = _sample_classes(
                val_dset,
                dataset_to_cls_idx[cur_dataset],
                dataset_to_cls_idx[cur_dataset] + max_classes_per_dataset,
            )
            tst_dset = _sample_classes(
                tst_dset,
                dataset_to_cls_idx[cur_dataset],
                dataset_to_cls_idx[cur_dataset] + max_classes_per_dataset,
            )
...
def get_datasets(
    ...
    if "mnist" in dataset:
        ...
        if dataset == "domainnet":
            _ensure_domainnet_prepared(
                path, classes_per_domain=nc_first_task, num_tasks=num_tasks
            )
        ...
        for dataset in dataset_list:
            ...
            for class_label in unique_labels:
                ...
                sampled_indices.extend(
                    np.random.choice(
                        class_indices, size=max_examples_per_class
                    ).tolist()
                )
...
def get_transforms(
    ...
    ):  # removes Gaussian Blur from Cifar augmentations as it does not work well
        simCLR_transforms = [
    ...
    if extra_aug == "fetril":  # Similar as in PyCIL
        ...
        if "cifar" in ds_name.lower():
            ...
            raise RuntimeError(
                f"Please check and update the data agumentation code for your dataset: {ds_name}"
            )
...
def _ensure_tinyimagenet_prepared(path):
    assert os.path.exists(
        path
    ), f"Please first download and extract dataset from: https://www.kaggle.com/competitions/tiny-imagenet to dir: {path}"
...
def _ensure_domainnet_prepared(path, classes_per_domain=50, num_tasks=6):
    assert os.path.exists(
        path
    ), f"Please first download and extract dataset from: http://ai.bu.edu/M3SDA/#dataset into:{path}"
