...
class Appr(Inc_Learning_Appr):
    ...
    def train_epoch(self, t, trn_loader):
        ...
        with GradCAM(self.model_old, self.gradcam_layer) as gradcam_old:
            for images, targets in trn_loader:
                ...
                with GradCAM(
                    self.model, self.gradcam_layer, retain_graph=True
                ) as gradcam:
                    attmap = gradcam(images)  # this use eval() pass
                self.model.zero_grad()
                self.model.train()
                outputs = self.model(images)  # this use train() pass
                loss, loss_ce, loss_kd, loss_atn = self.criterion(
                    t,
                    outputs,
                    targets,
                    outputs_old,
    ...
    def eval(self, t, val_loader):
        ...
        with GradCAM(self.model, self.gradcam_layer) as gradcam, GradCAM(
            ...
            for images, targets in val_loader:
                ...
                loss = self.criterion(
                    t, outputs, targets, outputs_old, attmap, attmap_old
                )
    ...
    def criterion(
        ...
        if t > 0 and outputs_old is not None:
            ...
            loss_kd = self.beta * self.cross_entropy(
