...
def plot_compare(data, output_path):
    ...
    plot = sns.scatterplot(
        x="cost", y="acc", hue="name", data=pd.DataFrame(ic_records), legend=False
    )
...
if __name__ == "__main__":
    ...
    for r in tqdm(result_dirs):
        ...
        if len(ee_filenames) == 1:
            ...
            output = {"run_name": run_name, "no_ee_acc": 100 * final_val}
            output_data.append(output)
    ...
    for p in tqdm(plot_prefixes, "Plotting early exit scores..."):
        method_data = {
            ...
            for method_name, method_data in plot_data.items()
            if method_name.startswith(p)
        }
        sorted_method_data = {
            k: v for k, v in sorted(method_data.items(), key=lambda item: item[0])
        }
